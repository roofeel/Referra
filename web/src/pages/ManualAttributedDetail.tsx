            ...
            <>
              ...
              <section className="rounded-xl border border-violet-200 bg-violet-50 p-5">
                <div className="flex items-center justify-between">
                  <h2 className="text-sm font-bold text-violet-950">Cron Configuration</h2>
                  <span className={`rounded px-2 py-1 text-[10px] font-black uppercase tracking-widest ${job.cronEnabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                    {job.cronEnabled ? 'Enabled' : 'Disabled'}
                  </span>
                </div>
                <div className="mt-3 grid gap-4 text-xs text-slate-700 md:grid-cols-3">
                  <div><p className="font-black uppercase tracking-widest text-slate-500">Expression</p><p className="mt-1 font-mono">{job.cronExpression || '-'}</p></div>
                  <div><p className="font-black uppercase tracking-widest text-slate-500">Timezone</p><p className="mt-1">Asia/Shanghai (or CRON_TIMEZONE)</p></div>
                  <div><p className="font-black uppercase tracking-widest text-slate-500">Variables</p><p className="mt-1 break-all font-mono">{JSON.stringify(job.cronVariables || {})}</p></div>
                </div>
              </section>
