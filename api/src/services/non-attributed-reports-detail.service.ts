import { nonAttributedRaws, nonAttributedReports, urlRules } from '../../../packages/db/index.js';
import {
  isAttributionMode,
...
type NonAttributedRawRecord = Awaited<ReturnType<typeof nonAttributedRaws.listByReport>>[number];
...
export type NonAttributedReportDetailPayload = {
  clientName: string;
...
function isMatchedRow(row: Pick<NonAttributedRawRecord, 'referrerType' | 'referrerDesc'>) {
...
function extractUidFromEventUrl(urlValue: string, uidParamName: string) {
  const parsed = parseUrl(urlValue);
  if (!parsed) return '--';

  const target = uidParamName.trim().toLowerCase() || 'uid';
  for (const [key, value] of parsed.searchParams.entries()) {
...
function toReferrerTypeStatsFromRows(rows: NonAttributedRawRecord[]) {
...
function parseTimeMsByCandidates(item: NonAttributedRawRecord, candidates: string[]) {
...
function buildDetailPayload(
  ...
  const rows: ReportDetailTableRow[] = rawRows.map((item) => {
    ...
    return {
      eventId: item.id,
      uid: extractUidFromEventUrl(eventUrl, report.uidParamName || 'uid'),
      eventName,
  ...
  rawRows.forEach((item) => {
    ...
    eventDetails[item.id] = {
      ...
      attributionPath: [
        ['Source', sourceUrl || '--', 'bg-emerald-500'],
        ['Event URL', urlValue, 'bg-blue-500'],
...
export async function getNonAttributedReportDetailPayload(
  report: NonNullable<NonAttributedReportRecord>,
  options: {
    page: number;
    pageSize: number;
  ...
  const [totalRows, rawRows, groupedByTypeRaw] = await Promise.all([
    nonAttributedRaws.countByReport(report.id),
    nonAttributedRaws.listByReport(report.id, { skip, take: options.pageSize }),
    nonAttributedRaws.countByReportGroupedType(report.id),
  ]);
  ...
  return buildDetailPayload(report, rule, rawRows as NonAttributedRawRecord[], {
    page: options.page,
    pageSize: options.pageSize,
